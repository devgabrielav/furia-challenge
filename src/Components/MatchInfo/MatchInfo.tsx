...
import { channelNameExtract, dateConvert, fetchMatches, getFullDate, matchTotal, matchType, pointsResult, timeConvert } from "../../utils/matches";
        ...
        <>
          <div className="firstItems">
            <div className="legends">
              <div>
                <p className="legendP" style={{ color: '#00CE00' }}>
                  W: Win (Time vencedor)
                </p>
                <p className="legendP" style={{ color: '#FF0000', marginLeft: '2px' }}>
                  L: Loser (Time perdedor)
                </p>
                <p className="legendP">BO: Best Of (Melhor de)</p>
              </div>
              <div>
                <p className="legendP" style={{ color: '#E1B833', width: '110px' }}>
              ...
              <div className="matchDiv" key={match.id}>
                <div className="matchNameDiv">
                  <span className="matchName">
                    {`${match.league.name} ${match.serie.full_name} ${match.tournament.name} - ${getFullDate(
                  ...
                  <div className="matchStatusInfoDiv">
                    <span
                      className={`matchStatus ${
                        match.status === 'running' ? 'running' : 'finished'
                      }`}
                    >
                      LIVE
                    </span>
                  ...
                  <div className="matchInfoDiv">
                    {match.opponents.length === 0 ? (
                      ...
                      <div className="dateTeamsDiv">
                        <div className="teamsResultsDiv">
                          ...
                          <span className="matchTotal">{matchTotal(match)}</span>
                    ...
                    <div className="liveStreamsDiv">
                      {match.streams_list
                        .sort(
                          (a, b) =>
                            channelNameExtract(b.raw_url).length - channelNameExtract(a.raw_url).length
                        )
                        .map((live) => {
                          ...
                          return (
                            <div className="livestreams" key={live.embed_url}>
                              <a className="liveStreamTag" href={live.raw_url} target="_blank">
                                ...
                                />
                                <span>
                                  {channelName.length > 7
                                    ? channelName.slice(0, 7)
                                    : channelName}
                                </span>
                              </a>
                            </div>
